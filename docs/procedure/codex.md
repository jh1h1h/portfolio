...
- Make sure that you are not sending more than one request per second to any endpoint
...
...and any other tools that are by default installed in Kali Linux. You may use tools not stated here, but please provide the commands (as well as the source for the tool and the installation steps) to properly set the tool up before stating the command for the tool that you want me to run.
